import type { DV, Iso8601 } from '@iamssen/exocortex';
import { findStartIndex } from '@iamssen/exocortex/date-utils';
import type {
  CartesianChartAxis,
  HistoryLineData,
  HistoryLineRecord,
  PriceLineData,
} from '@ui/cartesian-chart';
import { DateTime } from 'luxon';
import { chartAxis } from '../env.ts';

export interface PeChartData {
  history: HistoryLineData;
  historyBenchmark?: HistoryLineData;

  price: PriceLineData | undefined;
  watches: PriceLineData[];

  axis: CartesianChartAxis;
}

export interface PeChartParams {
  data: DV<number>[];
  benchmarkData?: DV<number>[];
  watches: PriceLineData[];

  start: Iso8601;
  end?: Iso8601;
}

export function createPeChartData({
  data,
  benchmarkData,
  watches,
  start,
  end,
}: PeChartParams): PeChartData {
  const records: HistoryLineRecord[] = data
    .slice(findStart(data, start))
    .map(({ date, value }) => ({
      date,
      timestamp: new Date(date).getTime(),
      value,
    }));

  const benchmarkRecords = benchmarkData
    ?.slice(findStart(benchmarkData, start))
    .map(({ date, value }) => ({
      date,
      timestamp: new Date(date).getTime(),
      value,
    }));

  const values: number[] = [
    ...watches.map(({ price }) => price),
    ...records.map(({ value }) => value),
  ];

  if (benchmarkRecords) {
    for (const { value } of benchmarkRecords) {
      values.push(value);
    }
  }

  const xmin = new Date(start).getTime();
  const xmax = end ? new Date(end).getTime() : Date.now();
  const ymin = Math.min(...values);
  const ymax = Math.max(...values);

  return {
    history: {
      formatString: undefined,
      records: [records],
    },
    historyBenchmark: benchmarkRecords
      ? {
          formatString: undefined,
          records: [benchmarkRecords],
        }
      : undefined,
    price: {
      formatString: undefined,
      price: records.at(-1)?.value ?? 0,
    },
    watches,
    axis: {
      x: {
        min: xmin * chartAxis.XMIN,
        max: xmax * chartAxis.XMAX,
      },
      y: {
        min: ymin * chartAxis.YMIN,
        max: ymax * chartAxis.YMAX,
      },
    },
  };
}

const findStart = findStartIndex<DV<number>>(({ date }) =>
  DateTime.fromISO(date),
);
